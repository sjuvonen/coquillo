...
QVariant FileBrowserModel::data(const QModelIndex &idx, int role) const {
  ...
  if (role == Qt::CheckStateRole) {
    bool contains =
        checked.contains(idx.data(QFileSystemModel::FilePathRole).toString());
...
bool FileBrowserModel::setData(const QModelIndex &idx, const QVariant &value,
                               int role) {
  if (role == Qt::CheckStateRole) {
    ...
    if (value.toBool()) {
      ...
      if (recursive) {
        ...
        for (const auto &path : erased) {
          updates << mapFromSource(
              inner->index(path, mapToSource(idx).column()));
        }
      }
    } else {
  ...
  while (!notify.empty()) {
    ...
    if (rowCount(target)) {
      ...
      const auto bottomRight =
          index(rowCount(target) - 1, target.column(), target);
...
bool FileBrowserModel::isDescendantChecked(const QModelIndex &idx) const {
  QString path =
      idx.data(QFileSystemModel::FilePathRole).toString() + QDir::separator();
