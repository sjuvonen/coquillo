...
void CddbSearchDialog::accept() {
	...
	for (int i = 0; i < _indexes.count(); i++) {
		...
		if (setAll || setArtist)
			_dataModel->setData(idx.sibling(idx.row(), MetaData::ArtistField), artist);

		if (setAll || setAlbum)
			_dataModel->setData(idx.sibling(idx.row(), MetaData::AlbumField), album);

		if (setAll || setGenre)
			_dataModel->setData(idx.sibling(idx.row(), MetaData::GenreField), info.genre);

		if (setAll || setNumber)
			_dataModel->setData(idx.sibling(idx.row(), MetaData::NumberField), i+1);

		if (setAll || setMaxNumber)
			_dataModel->setData(idx.sibling(idx.row(), MetaData::MaxNumberField), maxNum);

		if (setAll || setTitle)
			_dataModel->setData(idx.sibling(idx.row(), MetaData::TitleField), title);

		if (setAll || setYear)
			_dataModel->setData(idx.sibling(idx.row(), MetaData::YearField), info.year);
...
void CddbSearchDialog::setSelection(const QItemSelection & selection) {
	DataWidget::setSelection(selection);

	QList<QPersistentModelIndex> indexes = rows();
	_ui->labelInfo->clear();
	...
	for (int i = 0; i < indexes.count(); i++) {
		const QPersistentModelIndex idx = indexes[i].sibling(indexes[i].row(), MetaData::LengthField);
...
void CddbSearchDialog::showMatches(const CddbAlbumInfoList & firstPage, int total, int pages) {
	...
	if (total > 0)
		_ui->labelInfo->setText(tr("%1 result(s) on %2 page(s)").arg(total).arg(pages));
	else
